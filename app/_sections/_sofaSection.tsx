...
const SofaSection = (props: Props) => {
  ...
  return (
    <div id='sofa-section-container' className='h-screen' style={{scrollSnapAlign: 'start'}}>
        ...
        <div className='w-1/2 relative flex items-center h-full bg-white transition-opacity duration-500'>
          {sofas.map((sofa) => (
            <Image
              ...
              src={sofa.src}
              alt={sofa.alt}
              className={`transition-all duration-1000 absolute ${sofa.id <= currentImage ? 'opacity-100' : 'opacity-0'
                }`}
            />
          ))}
          <div className='flex gap-1 absolute bottom-9 left-[50%] translate-x-[-50%]'>
            <RoundedShadowButton callback={() => setCurrentImage(1)} button_color='rgb(189,108,21, 0.5)' shadow_color='rgb(189,108,21, 1)' />
            <RoundedShadowButton callback={() => setCurrentImage(2)} button_color='rgb(67,62,153, 0.5)' shadow_color='rgb(67,62,153, 1)' />
            <RoundedShadowButton callback={() => setCurrentImage(3)} button_color='rgb(180,172,55, 0.5)' shadow_color='rgb(180,172,55, 1)' />
          </div>
        </div>
        <div
